...
class RepConvPruner(tp.pruner.BasePruningFunc):
    def prune_out_channels(self, layer: RepConv, idxs: Sequence[int]):
        keep_idxs = list(set(range(layer.c2)) - set(idxs))
        keep_idxs.sort()
        layer.c1 = layer.c1-len(idxs)
        layer.c2 = layer.c2-len(idxs)
        layer.g = layer.g-len(idxs)
        
        layer.conv1.conv.weight = self._prune_parameter_and_grad(layer.conv1.conv.weight, keep_idxs, 0)
        layer.conv2.conv.weight = self._prune_parameter_and_grad(layer.conv2.conv.weight, keep_idxs, 0)
